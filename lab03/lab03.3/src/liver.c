...
/* list adapter implementations */
void delete_liver(Liver l) {
    (void)l; /* nothing to free */
}
...
int calculate_age(const Liver *l) {
    ...
    get_today(&dd, &mm, &yy);
    int age = yy - l->year;
    if (mm < l->month || (mm == l->month && dd < l->day)) age--;
    return age;
}

/* compare_by_age_for_insert: younger first (smaller age -> earlier) */
...
/* parse a line (tries with patronymic and without) */
int parse_line_to_liver(const char *line, Liver *out) {
    ...
    int scanned9 = sscanf(line, "%u %127s %127s %127s %d %d %d %c %lf",
                          &id_u, s1, s2, s3, &day, &month, &year, &gender, &income);
    if (scanned9 == 9) {
        out->id = id_u;
        strncpy(out->surname, s1, sizeof(out->surname)-1); out->surname[sizeof(out->surname)-1] = '\0';
        strncpy(out->name, s2, sizeof(out->name)-1); out->name[sizeof(out->name)-1] = '\0';
        strncpy(out->patronymic, s3, sizeof(out->patronymic)-1); out->patronymic[sizeof(out->patronymic)-1] = '\0';
    ...
    int scanned8 = sscanf(line, "%u %127s %127s %d %d %d %c %lf",
                          &id_u, s1, s2, &day, &month, &year, &gender, &income);
    if (scanned8 == 8) {
        out->id = id_u;
        strncpy(out->surname, s1, sizeof(out->surname)-1); out->surname[sizeof(out->surname)-1] = '\0';
        strncpy(out->name, s2, sizeof(out->name)-1); out->name[sizeof(out->name)-1] = '\0';
...
/* insert_sorted_by_age operates on Liver_List (generated by macro).
   It inserts keeping list ordered by (increasing) age as required. */
void insert_sorted_by_age(Liver_List *list, Liver val) {
    if (list->size == 0) {
        Liver_push_front_list(list, val);
        return;
    }
    /* traverse until we find node older than val (cmp > 0) -> insert before */
...
/* read_from_file / save_to_file / display_residents / search_residents
   — kept as non-static so main can call them. */
...
void display_residents(const Liver_List *list) {
    printf("\n--- Список (%zu) ---\n", list->size);
    if (list->size == 0) { printf("Пусто\n"); return; }
    Liver_Node *cur = list->head;
    while (cur) {
        Liver lv = *(cur->data);
        int age = calculate_age(&lv);
        printf("ID=%u | %s %s %s | %02d.%02d.%04d | age=%d | %c | income=%.2lf\n",
               lv.id, lv.surname, lv.name, lv.patronymic,
...
/* search_residents is left to main.c to declare (we will keep original as static in main.c)
   but we provide a wrapper if needed. For simplicity search_residents implemented in main.c. */
