...
typedef struct{\
    ...
    int (*Compare)(LIST_TYPE, LIST_TYPE);/*сравнение*/\
    LIST_TYPE invalidData; \
} NAME##_List;\
\
static inline NAME##_List NAME##_create_list( void (*DeleteVoidPtr)(LIST_TYPE),\
        LIST_TYPE (*CopyVoidPtr)(LIST_TYPE), int (*Compare)(LIST_TYPE, LIST_TYPE), LIST_TYPE invalidData){\
    NAME##_List l;\
    l.head = NULL; \
    l.tail = NULL; \
    l.size = 0; \
...
static inline NAME##_Node *NAME##_create_node(NAME##_List *list, LIST_TYPE value){\
    NAME##_Node *node = (NAME##_Node *)malloc(sizeof(NAME##_Node));/*выделяем память для узла*/\
    if (!node) return NULL; \
    node->data = (LIST_TYPE*)malloc(sizeof(LIST_TYPE));/*выделяем память для данных узла*/\
    if (!node->data){\
...
static inline NAME##_Node* NAME##_get_node_at(const NAME##_List *list, size_t index){\
    ...
    NAME##_Node *found;\
    /*для оптимизации ищем с начала до середины или с конца до середины*/\
